...
export default function Login({ onSuccess, onSwitch }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setError('Proses masuk gagal. Silakan periksa kembali kredensial Anda.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <main className="mx-auto flex min-h-[70vh] w-full max-w-5xl items-center justify-center px-6 pb-16">
      <section className="w-full max-w-md rounded-3xl border border-slate-800/70 bg-slate-900/70 p-8 shadow-sm">
        <h2 className="text-2xl font-semibold text-white">Masuk ke Akun</h2>
        <p className="mt-2 text-sm text-slate-400">Silakan masuk untuk mengelola jalur karir Anda.</p>
